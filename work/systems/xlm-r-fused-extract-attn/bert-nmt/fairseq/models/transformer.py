...
class TransformerS2Model(FairseqEncoderDecoderModel):
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, bert_input, **kwargs):
        ...
        """
        bert_encoder_padding_mask = bert_input.eq(self.berttokenizer.get_vocab()[self.berttokenizer.pad_token])
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        for layer in self.layers:
            x, attn_maps_at = layer(x, encoder_padding_mask)
            attn_maps_list.append(attn_maps_at)
        attn_maps_standard_encoder = None
        if attn_maps_list is not None:
            attn_maps_standard_encoder = torch.stack(attn_maps_list, dim=0) #(layers, batch, heads, tgt_len, src_len)
            attn_maps_standard_encoder = attn_maps_standard_encoder.transpose(0, 1).contiguous().transpose(3, 4).contiguous()
        if self.layer_norm:
            x = self.layer_norm(x)
        ...
        return {
            ...
            'attn_maps_standard_encoder': attn_maps_standard_encoder
        }
...
class TransformerS2Encoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, bert_encoder_out):
        ...
        # encoder layers
        for layer in self.layers:
            x, self_attn_S2, bert_attn_S2 = layer(x, encoder_padding_mask, bert_encoder_out['bert_encoder_out'], bert_encoder_out['bert_encoder_padding_mask'])
            self_attn_S2_list.append(self_attn_S2)
            bert_attn_S2_list.append(bert_attn_S2)
        self_attn_S2_tensor = None
        bert_attn_S2_tensor = None

        if self_attn_S2_list is not None:
            self_attn_S2_tensor = torch.stack(self_attn_S2_list, dim=0)  # (layers, batch, heads, tgt_len, src_len)
            self_attn_S2_tensor = self_attn_S2_tensor.transpose(0, 1).contiguous()
        if bert_attn_S2_list is not None:
            bert_attn_S2_tensor = torch.stack(bert_attn_S2_list, dim=0)  # (layers, batch, heads, tgt_len, src_len)
            bert_attn_S2_tensor = bert_attn_S2_tensor.transpose(0, 1).contiguous()
        if self.layer_norm:
            x = self.layer_norm(x)
        ...
        return {
            'encoder_out': x,  # T x B x C
            'encoder_padding_mask': encoder_padding_mask,  # B x T
            'self_attn_S2_tensor': self_attn_S2_tensor,
            'bert_attn_S2_tensor': bert_attn_S2_tensor
        }
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        x, attn_maps_at = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
...
class TransformerS2EncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask, bert_encoder_out, bert_encoder_padding_mask):
        ...
        residual = x
        x = self.maybe_layer_norm(self.self_attn_layer_norm, x, before=True)
        x1, self_attn_S2 = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask, need_weights=True)
        x2, bert_attn_S2 = self.bert_attn(query=x, key=bert_encoder_out, value=bert_encoder_out, key_padding_mask=bert_encoder_padding_mask, need_weights=True)
        ...
        x = residual + x
        x = self.maybe_layer_norm(self.final_layer_norm, x, after=True)
        return x, self_attn_S2, bert_attn_S2
