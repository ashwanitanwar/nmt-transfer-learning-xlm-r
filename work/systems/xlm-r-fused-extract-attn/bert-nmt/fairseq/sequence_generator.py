...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        bert_encoder_padding_mask = bertinput.eq(model.models[0].berttokenizer.get_vocab()[model.models[0].berttokenizer.pad_token])
        ...
        self_attn_S2_tensor = encoder_outs[0]['self_attn_S2_tensor']
        bert_attn_S2_tensor = encoder_outs[0]['bert_attn_S2_tensor']
        if(self_attn_S2_tensor is not None and bert_attn_S2_tensor is not None):
            #Convert to Numpy according to batch_ids and save them.
            batch_ids = sample['other']['batch_ids']
            sort_batch_ids = numpy.argsort(batch_ids)
            save_attn_maps = sample['other']['save_attn_maps']
            src_dict = sample['other']['src_dict']
            remove_bpe = sample['other']['remove_bpe']
            src_str_unsorted = src_dict.string(src_tokens, bpe_symbol=remove_bpe)
            src_tokens_sorted = torch.from_numpy(src_tokens.cpu().numpy()[sort_batch_ids])
            src_str_sorted = src_dict.string(src_tokens_sorted, bpe_symbol=remove_bpe)
            self.batch_count += 1
            self_attn_S2_tensor_np = self_attn_S2_tensor.cpu().numpy()
            bert_attn_S2_tensor_np = bert_attn_S2_tensor.cpu().numpy()
            self_attn_S2_tensor_np_sorted = self_attn_S2_tensor_np[sort_batch_ids]
            bert_attn_S2_tensor_np_sorted = bert_attn_S2_tensor_np[sort_batch_ids]
            numpy.save(save_attn_maps+'/self/'+ str(self.batch_count), self_attn_S2_tensor_np_sorted )
            numpy.save(save_attn_maps + '/bert/' + str(self.batch_count), bert_attn_S2_tensor_np_sorted)
            with open(save_attn_maps+'/batch_sentences/'+ str(self.batch_count)+'.txt', 'w') as batch_file:
                batch_file.write('Sorted *******:\n'+src_str_sorted+'\n\n\n Unsorted *******:\n'+src_str_unsorted)
